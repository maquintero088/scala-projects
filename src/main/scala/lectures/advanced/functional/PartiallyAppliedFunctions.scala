...
object PartiallyAppliedFunctions extends App {
  ...
  val add5 = CurriedFunctions.curriedAdder(5) _ // Int => Int
  ...
  // x: String => concatenator(hello, x, howarewyou)
  ...
  byName(parenMethod())
  // byName(parenMethod)
  // Scala 2: ok but beware ==> byName(parenMethod()); Scala 3 forbids calling the method with no parens
  //  byName(() => 42) // not ok
  byName((() => 42)()) // ok
  ...
  //  byFunction(method)
  // not ok!!!!!! does not do ETA-expansion!
  byFunction(parenMethod)
  // compiler does ETA-expansion
  byFunction(() => 46)
  // works
  byFunction(parenMethod _)
  // also works, but warning- unnecessary
}
